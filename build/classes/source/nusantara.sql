...
CREATE TABLE member(
	...
	agama ENUM('Islam', 'Kristen', 'Katolik', 'Buddha', 'Hindu', 'Konghuchu'),

	FOREIGN KEY (province_id) REFERENCES provinces(id) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (regencies_id) REFERENCES regencies(id) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (districts_id) REFERENCES districts(id) ON UPDATE CASCADE ON DELETE CASCADE
);
...
VALUES (
    'UP-23-001',         -- member_id (following the UP-YY-XXX format)
    'John Doe',          -- nama
    '1990-05-15',        -- tanggal_lahir (in YYYY-MM-DD format)
    '123 Main St',       -- alamat
    11,                   -- province_id (must refer to an existing id in provinces table)
    1101,                  -- regencies_id (must refer to an existing id in regencies table)
    1101010,                 -- districts_id (must refer to an existing id in districts table)
    '081234567890',      -- nomor_telepon
    'l',                 -- jenis_kelamin (l for male, p for female)
    'Islam'              -- agama (one of the allowed ENUM values)
);


SELECT * FROM regencies;

-- Active: 1722388680842@@localhost@3306@penduduk
